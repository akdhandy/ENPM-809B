...
//    bin2_.left_arm = {0.13,-0.42,0.5,-0.1,1.70,0}; -- working
    ...
    bin15_.left_arm = {0,-0.5,0.9,-0.4,1.53,0};
...
//    shelf 2
    ...
    logicam15ra_.right_arm = {PI, -PI/4, PI/2, -PI/4, PI/2, 0};
...
//  Shelf preset locations
    ...
    if (Human_there==true)
    {
        ...
        if (Human[1]==0)
        {
            ...
            {
                if(gap_nos[0]==34){
                    presetLocation["logical_camera_7_frame_left"] = {lc7ra_,lc5ld2_,lc5ld1_,lc6la_,lc7l_};
                    presetLocation["logical_camera_6_frame_left"] = {lc7ra_,lc5ld2_,lc5ld1_,lc6la_,lc6lb_};
                }
                else if(gap_nos[0]==45){
                    presetLocation["logical_camera_7_frame_left"] = {lc7ra_,lc5ld3_,lc5ld_,lc6la_,lc7l_};
        ...
        else if ((x > -14.4 && x < -12.65) && (y > 0.16 && y < 0.5))
        {
            ...
            if (dir==1){
                ...
                for (auto i=0; i<vec.size(); i++)
                    ...
                    for (auto i=0; i<vec.size(); i++){
                        goToPresetLocation(vec[i]);
                        if (i==2 && comp.gap_nos[0]==45)
                            do{
                                comp.breakbeam_sensing();
                                ROS_INFO_STREAM("Waiting for human to move from the required position at gap 45..");
                            }while(!((comp.beam_seq[10]<comp.beam_seq[9]) && (comp.beam_seq[9]>comp.beam_seq[8]) && (comp.beam_seq[10]>comp.beam_seq[8])));
                        else if (i==2 && comp.gap_nos[0]==34)
                            do{
                                comp.breakbeam_sensing();
                                ROS_INFO_STREAM("Waiting for human to move from the required position at gap 34..");
                            }while(!((comp.beam_seq[9]<comp.beam_seq[8]) && (comp.beam_seq[8]>comp.beam_seq[7]) && (comp.beam_seq[9]>comp.beam_seq[7])));
                    }
            }
            else
                for (auto i=vec.size()-1; i>=0; i--)
